...
class _AdminGeofenceScreenState extends State<AdminGeofenceScreen> {
  ...
  Future<void> _saveAllPending() async {
    ...
    final msg = failedNames.isEmpty
        ...
        : 'Saved $successCount geofence(s). Failed: ${failedNames.take(3).join(', ')}${failedNames.length > 3 ? 'ΓÇª' : ''}';
  ...
  void _selectPlaceResult(_PlaceSearchResult result) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('≡ƒôì Map moved to: ${result.displayName}'),
              ...
              FilledButton(
                onPressed: () {
                  ...
                  if (dialogSelectedLabelLetter == null) {
                    setStateDialog(() {
                      dialogError = 'Please select a label letter (AΓÇôZ).';
                    });
                    return;
                  }
    ...
    for (int i = 0; i < active.length; i++) {
      for (int j = i + 1; j < active.length; j++) {
        if (_isOverlap(active[i], active[j])) {
          pairs.add('${active[i].name} Γåö ${active[j].name}');
        }
      }
    }
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: pairs.isEmpty
            ...
            : Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: pairs.map((p) => Text('ΓÇó $p')).toList(),
              ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
