...
// Admin only. Returns active employees scoped to the admin's company.
...
router.get('/employees', protect, requireRole('admin'), async (req, res) => {
  try {
    ...
    console.error('GET /api/tickets/employees error:', err);
...
router.get('/:id', protect, async (req, res) => {
  try {
    ...
    // Employee access guard — employees can only view their own assigned tickets
    if (req.user.role === 'employee') {
      const assigneeId = ticket.assignee
        ? String(ticket.assignee._id || ticket.assignee)
        : null;
      if (assigneeId !== String(req.user._id)) {
        return res.status(403).json({ message: 'Access denied' });
      }
    }
...
router.post('/', protect, async (req, res) => {
  try {
    ...
    // Validate assignee_id — must be an existing employee
    if (assignee_id) {
      const assignee = await User.findById(assignee_id).lean();
      if (!assignee) {
        return res.status(400).json({ message: 'Assignee not found' });
      }
      if (assignee.role !== 'employee') {
        return res.status(400).json({ message: 'Assignee must have role: employee' });
      }
    }
...
router.patch('/:id', protect, async (req, res) => {
  try {
    ...
    // Guard: reject updates on closed or verified tickets
    if (ticket.status === 'closed' || ticket.status === 'verified') {
      return res.status(400).json({
        message: `Cannot update a ticket with status '${ticket.status}'`,
      });
    }

    const { data, notes, assignee_id } = req.body;
    const previousAssignee = ticket.assignee ? String(ticket.assignee) : null;
    ...
    // Determine audit action and emit Socket.IO event if assignee changed
    const newAssignee = ticket.assignee ? String(ticket.assignee) : null;
    const assigneeChanged = assignee_id !== undefined && previousAssignee !== newAssignee;
    ...
    if (assigneeChanged) {
      ...
      if (global.io) {
        global.io.emit('ticketAssigneeChanged', {
          ticketId: String(ticket._id),
...
router.patch('/:id/status', protect, async (req, res) => {
  try {
    ...
    // Guard: closed tickets cannot be transitioned further
    if (oldStatus === 'closed') {
      return res.status(400).json({
        message: 'Cannot transition a closed ticket',
      });
    }
